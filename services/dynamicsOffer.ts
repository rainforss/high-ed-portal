  ...
  return {
    ...
    getOfferByOfferId: async (offerId: string) => {
      const offer = await retrieve(
        ...
        "bsi_offers",
        offerId,
        "$select=bsi_name,bsi_expirationdate,bsi_offerstatus&$expand=bsi_Program($select=mshied_name),bsi_StudentApplication($select=bsi_name,bsi_commitmentfeepaid,bsi_commitmentfeepaymentlink),bsi_PrerequisiteProgram($select=mshied_name,bsi_commitmentfee),bsi_AcademicPeriod($select=mshied_name)",
        { representation: true }
      );
